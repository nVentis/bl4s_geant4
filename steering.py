...
## 
##   SIM.action.mapActions['tpc'] = "TPCSDAction"
## The SiPM is only ever hit by optical photons (no charged particles reach
## it), so it needs the specialized optical tracker action instead of the
...
##     
SIM.edm4hep.eventHeaderCollectionName = "EventHeader"

## Set the name of the collection containing the MCParticle input.
##     
SIM.edm4hep.mcParticleCollectionName = "MCParticles"
...
## 1 keV cut, so the default filter would silently drop nearly every SiPM
## hit. LeadGlassBar's dE/dx hits are unaffected (GeV-scale, well above 0).
...
## +z, matching the LeadGlassBar/SiPM layout in GlassBar_Cherenkov.xml
## (bar spans z=0..50cm, SiPM face at z=50cm).
...
## Midpoint of the 2-6 GeV scan range; override per-run with
## `ddsim --steeringFile steering.py --gun.energy "2*GeV"` etc.
...
SIM.outputConfig.useRNTuple = False
...
## Disabled: this handler's cuts (e.g. minimalKineticEnergy, MeV-scale)
## interfere with associating eV-scale optical photons to their hits.
...
## Cerenkov + optical-photon physics are not part of any stock physics
## list and must be added explicitly. Pattern follows
## DD4hep/examples/RICH/scripts/richsim.py.
def setupCerenkov(kernel):
  from DDG4 import PhysicsList

  seq = kernel.physicsList()
  cerenkov = PhysicsList(kernel, 'Geant4CerenkovPhysics/CerenkovPhys')
  ## Caps photons generated per step; Geant4 subdivides the step instead of
  ## dropping photons if the expected yield is higher. ~50cm of LeadGlass at
  ## n=1.47 yields O(10^4) photons per traversing electron, so simulation
  ## will run noticeably slower with optical physics enabled than without.
...
SIM.physics.rejectPDGs = {1, 2, 3, 4, 5, 4101, 6, 4103, 3201, 3203, 21, 23, 24, 5401, 25, 5403, 2203, 3101, 3103, 4403, 2101, 5301, 2103, 5303, 4301, 1103, 4303, 5201, 5203, 3303, 4201, 4203, 5101, 5103, 5503}
